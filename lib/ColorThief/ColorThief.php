...
class ColorThief
{
    ...
    public static function getPalette($sourceImage, $colorCount = 10, $quality = 10, array $area = null)
    {
        ...
        for($i=0;$i<$colorCount;$i++){
            $result = (array)($cmap->vboxes->contents);
            $vbox = $result[$i]['vbox'];
            $p[$i] = $vbox->volume*$vbox->count;
            $sum+=$p[$i];
        }
        ...
        $palette = $cmap->palette($p);

        for($i=1,$j=0;$i<$colorCount*2;$i+=2,$j++){
            $palette = static::array_insert((array)$palette,$i,array($p[$j]));
        }

        return $palette;
    }

    private static function array_insert ($array, $position, $insert_array) {
